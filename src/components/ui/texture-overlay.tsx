...
interface TextureOverlayProps {
  texture: TextureType
  opacity?: number
  /** Grid cell size in px (only applies when texture="grid"). Default 12. */
  gridSize?: number
  className?: string
}
...
const texturePatterns: Record<TextureType, string> = {
  ...
  grid: "bg-[linear-gradient(rgba(0,0,0,0.3)_1px,transparent_1px),linear-gradient(90deg,rgba(0,0,0,0.3)_1px,transparent_1px)]",
  noise:
...
export function TextureOverlay({
  ...
  const style: Record<string, string | number> = {
    ...
    ...(texture === "grid" && { backgroundSize: `${gridSize}px ${gridSize}px` }),
  }
